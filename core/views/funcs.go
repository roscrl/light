...
var TemplateFuncs = template.FuncMap{
	...
	"formatNumberInK": formatNumberInK,
...
func pluralize(count any, singular, plural string) (string, error) {
	n, err := toInt64(count)
	if err != nil {
		return "", err
	}
...
func formatNumberInK(num int64) string {
	const K = 1000
	if num >= K {
		quotient := num / K
...
func toInt64(i any) (int64, error) {
	...
	case string:
		...
		if err != nil {
			return 0, fmt.Errorf("unable to convert string %q to int: %w", v, err)
		}
